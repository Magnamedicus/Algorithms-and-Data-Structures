...
class LinkedList:
    ...
    def find_middle_node(self):
        if self.head == None:
            ...
            print("This list contains only 1 node")
            return self.head
            ...
            print(f"The middle node is {slow_pointer.value}")

            return slow_pointer


#The find_middle_node() method will find and return the node that is between the list's two halves if it is odd numbered
#or the first node of the list's second half if it is even numbered.
#the method first sets two pointers equal to self.head.
#we then use a while loop to iterate the pointers forward. Each round of the loop will move
#'fast_pointer' two nodes forward and 'slow_pointer' one node forward
#this will continue until 'fast_pointer' is either at the end node or has moved off the end and is equal to None

#we then return the node where 'slow_pointer' is currently located
